    ...
    def apply(attackPlayer: Player): Winner = attackPlayer match {
      case _: Human =>
        ...
        selectedAction match {
          ...
          case "2" =>
            if (attackPlayer.allCardsValue >= 30) new Winner(attackPlayer, Some(true))
            else new Winner(attackPlayer, Some(false))
        }

      case robot: Robot if robot.getTricksPoints >= 30 => new Winner(attackPlayer, Some(true))
      case _                                           => new Winner(attackPlayer)
    }
