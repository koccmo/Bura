...
object RobDefender {
  ...
  def defend(
    ...
    val defHasAttSuitAndCanBeat: Boolean = {
      ...
      getCardStrength(cardSimilarSuitAttack).exists(defStr => getCardStrength(attackCard).exists(attStr => defStr > attStr))
    }
    ...
    val canBeat: Boolean = {
      ...
      def defenderBeatsAttacker(attackerCard: List[Card], defenderCards: List[Card]): Boolean = {
        ...
        def defenderHasMoreCards: Boolean = {
          val defenderCombinationAsAttackerAmount: List[List[Card]] = defenderCards.combinations(attackerCard.size).toList
          val ok: List[Int] =
    ...
    def bothOneTrump(): List[Card] = {
      ...
      else if (attackCardsPoints == 0 && defTrumpsPointSum == 0 && defNoTrumpsPointSum <= 15 && cardsDifferentSuit == 3) defNoTrumpsCards.drop(attackCardAmount) // drop
      else if (attackCardsPoints == 0 && defTrumpsPointSum >= 2 && defTrumpsPointSum <= 4 && cardsDifferentSuit == 2) defTrumpCards //take
      else if (attackCardsPoints == 0 && defTrumpsPointSum >= 2 && defTrumpsPointSum <= 4 && cardsDifferentSuit == 3) defNoTrumpsCards.drop(attackCardAmount) //drop
      else if (attackCardsPoints == 0 && defTrumpsPointSum <= 10 && defNoTrumpsPointSum == 21) defTrumpCards //take
      else if (attackCardsPoints == 0 && defTrumpsPointSum <= 10 && defNoTrumpsPointSum <= 15 && cardsDifferentSuit == 2) defTrumpCards //take
      else if (attackCardsPoints == 0 && defTrumpsPointSum <= 10 && defNoTrumpsPointSum <= 15 && cardsDifferentSuit == 3) defNoTrumpsCards.drop(attackCardAmount) //drop
      ...
      else if (attackCardsPoints >= 2 && defNoTrumpsPointSum <= 15 && cardsDifferentSuit == 3) defNoTrumpsCards.drop(attackCardAmount) //drop
      else List.empty
    }
    ...
    def bothNoTrumps(): List[Card] = {
      ...
      else if (attackCardAmount == 1 && cardsDifferentSuit == 2) defCard.filter(_.suit == attackCardSuit).drop(attackCardAmount) //take
    ...
    def defenderHasTrumps(): List[Card] = {
      def oneAtcOneTrumpDef(): List[Card] = {
        ...
        else if (attackCardsPoints == 10 && cardsDifferentSuit == 3 && defHasAttSuitAndCanBeat) defNoTrumpsCards.filter(_.suit == attackCardSuit) //take
        else if (attackCardsPoints <= 4 && cardsDifferentSuit == 3 && defHasAttSuitAndCanBeat) defNoTrumpsCards.filter(_.suit == attackCardSuit) //take
        else defTrumpCards
      }
      ...
      def attackTwoDefenderOneTrump(): List[Card] = {
        ...
        if (cardsDifferentSuit == 3 && defHasAttSuitAndCanBeat) defNoTrumpsCards.filter(_.suit == attackCardSuit) ++ defTrumpCards // take
        ...
        else if (cardsDifferentSuit == 2 && amountCanBeatNotTrump == 2) defTrumpCards :+ defNoTrumpsCards.maxBy(_.rank.strength) // take
        else if (cardsDifferentSuit == 2 && amountCanBeatNotTrump == 1) defTrumpCards :+ defNoTrumpsCards.maxBy(_.rank.strength) // take
        else defTrumpCards :+ defNoTrumpsCards.maxBy(_.rank.strength) //take
      }

      def attackTwoDefenderTwoTrump(): List[Card] = {//need fix this not work proporly
        if (getCardStrength(attackCard).count(_ < getCardStrength(defNoTrumpsCards).sum) >= 1) defNoTrumpsCards :+ defTrumpCards.minBy(_.rank.strength) // take
    ...
    def cantBeat(): List[Card] = {
      ...
      def oneAttack(): List[Card] = {
        if (!defHasTrump && cardsDifferentSuit == 3) defCard.minByOption(_.rank.strength).map(List(_)).getOrElse(List.empty[Card])
        else if (!defHasTrump && cardsDifferentSuit == 2) defCard.groupBy(_.suit).filter { case (_, cards) => cards.size == 1 }.toList.flatMap { case (_, cards) => cards }
        else if (defHasTrump && cardsDifferentSuit == 3 && defNoTrumpsPointSum <= 14) defNoTrumpsCards.drop(attackCardAmount)
        else if (defHasTrump && cardsDifferentSuit == 2 && defTrumpsPointSum >= 10) defNoTrumpsCards.drop(attackCardAmount)
        else defCard.sortBy(_.rank.points).take(1)
      }

      def twoAttack(): List[Card] = {
        if (!defHasTrump && cardsDifferentSuit == 3) defCard.drop(1)
        else if (!defHasTrump && cardsDifferentSuit == 2) defNoTrumpsCards.sortBy(_.rank.points).take(2)
        else if (defHasTrump && defTrumpAmount == 2) defNoTrumpsCards :+ defTrumpCards.minBy(_.rank.points)
