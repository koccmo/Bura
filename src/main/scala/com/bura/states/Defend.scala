...
trait Defend {
 def defend(attackCard: List[Card], players: Player, cardDesk: CardDesk): List[Card]
}

object RobDefend extends Defend {
  def defend(attackCard: List[Card], rob: Player, cardDesk: CardDesk): List[Card] = {
    ...
    def defenderHasTrumps(): List[Card] = {
      ...
      def attackTwoDefenderTwoTrump(): List[Card] = {
        if (getCardStrength(attackCard).count(_ < getCardStrength(defNoTrumpsCards).sum) >= 1) defNoTrumpsCards :+ defTrumpCards.minBy(_.rank.strength) // take need fix
