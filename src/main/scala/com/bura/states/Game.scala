...
//    println(s"DefendPlayer -> $defendPlayer")
      ...
      def apply(attacker: Player): Winner = attacker match {
        case _: Human =>
          ...
          selectedAction match {
            ...
            case "2" =>
              if (attacker.allCardsValue >= 30) Winner(attackerPlayer, Some(true)) else Winner(attacker, Some(false))
          }
      ...
      case Some(value) if value =>
        roundEnd.attacker match {
          case human: Human =>
            ...
            Game().play(robot, human, newCardDesk)
    ...
    def bothOpenCard(attacker: Player, defender: Player): Game = {
      ...
      if (attackerPoints > defenderPoints) attacker match {
        case human: Human =>
          Game().play(Robot(attack = false), Human(name = human.name, attack = true), CardDesk().setUpTrump)
        case _: Robot     =>
          Game().play(
            Robot(),
            Human(name = human.name),
            CardDesk().setUpTrump
          ) //Todo need make each player gameWins: Int = 0
      }
      ...
      else Game().play(Robot(), Human(human.name), CardDesk().setUpTrump)
    }
